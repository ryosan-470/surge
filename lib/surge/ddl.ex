defmodule Surge.DDL do
  ...
  defp update_attribute_defnitions(request_body, attribute_definitions, describe_table) do
    add_attributes_defnitions =
      Enum.reject(attribute_definitions, fn {key, _} ->
        Enum.member?(exsists_attribute_names(describe_table), key)
      end)
  ...
  defp exsists_attribute_names(describe_table) do
    ...
    # => [:id, :time]
    |> Enum.map(&String.to_atom(&1["AttributeName"]))
  end
  ...
  defp update_global_secondary_indexes(request_body, global_indexes, describe_table) do
    case describe_table["GlobalSecondaryIndexes"] do
      nil ->
        global_indexes_update_body(
          request_body,
          create_index_body_in_global_indexes_update(global_indexes)
        )

      exists_indexes_by_describe_table when is_list(exists_indexes_by_describe_table) ->
        exists_indexes =
          exists_indexes_by_describe_table
  ...
  defp pk_schema(hash: {hname, _htype}, range: {rname, _rtype}),
    do: [{hname, :hash}, {rname, :range}]
  ...
  defp encode_key_definitions(attrs) do
    ...
    |> Enum.map(fn {name, type} ->
      %{
        "AttributeName" => name,
        "AttributeType" => type |> ExAws.Dynamo.Encoder.atom_to_dynamo_type()
      }
    end)
  end
end
