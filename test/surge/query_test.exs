defmodule Surge.QueryTest do
  ...
  test "expression_attribute_names" do
    ...
    assert ["#id": "id", "#time": "time"] ==
             Surge.Query.expression_attribute_names(key_condition_expression, HashRangeModel)
  end
  ...
  test "build_query, select type count" do
    expect = %{
      ...
      "ExpressionAttributeValues" => %{":value1" => %{"N" => "2"}, ":value2" => %{"N" => "100"}},
  ...
  test "build_query + filter, select type count" do
    expect = %{
      "ExpressionAttributeNames" => %{"#id": "id", "#time": "time", "#age": "age"},
  ...
  test "query" do
    ...
    result =
      Surge.Query.query(
        where: ["#id = ? and #time >= ?", 2, 100],
        for: HashRangeModel,
        offset: [id: 2, time: 100]
      )
    ...
    result =
      Surge.Query.query(where: ["#id = ? and #time >= ?", 2, 100], for: HashRangeModel, limit: 1)
  ...
  test "build_scan_query" do
    ...
    query_param =
      Surge.Query.build_scan_query(["#id = ? and #time >= ?", 2, 100], HashRangeModel, nil)
  ...
  test "scan" do
    ...
    result =
      Surge.Query.scan(filter: ["#id = ? and #time >= ?", 2, 100], for: HashRangeModel, limit: 1)
